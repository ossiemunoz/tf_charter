...
% Estilos TikZ
\tikzstyle{fase}      = [rectangle, rounded corners, minimum width=3.4cm,
                         minimum height=1.1cm, text centered, draw=black, fill=blue!20]
\tikzstyle{resaltada} = [fase, fill=orange!35]
...
La compañía \empclientename\hspace{1px} ofrece servicios de \eng{project management} para la planificación, gestión, control y entrega de proyectos de tecnologías de la información bajo los estándares y mejores prácticas dictadas por el PMI (\eng{Project Management Institute}), desarrolladas en su guía PMBOK (\eng{Project Management Body Of Knowledge}).

Dado el ciclo de vida de un proyecto, establecido en 5 fases según los lineamientos del PMI representados en la figura 1, el alcance de este proyecto se circunscribe a las fases 3 de ejecución, y 4 de monitoreo y control:

\begin{figure}[h]
  \centering

  % Evitar conflicto babel (<,>) dentro del dibujo
  \shorthandoff{"}

  \begin{tikzpicture}[node distance=2.6cm]

    % Nodos (fases PMI/PMBOK)
    \node (inicio) [fase] {Inicio};
    \node (plan)   [fase, right of=inicio, xshift=5.2cm] {Planificación};
    \node (exec)   [resaltada, below of=plan, yshift=-2.2cm] {Ejecución};
    \node (mon)    [resaltada, left of=exec, xshift=-5.2cm] {Monitoreo y Control};
    \node (close)  [fase, below of=mon, yshift=-2.2cm] {Cierre};
  ...
  % Restaurar los atajos de babel
  \shorthandon{"}

  \caption{Ciclo de vida de un proyecto según PMI/PMBOK. Fases 3 (Ejecución) y 4 (Monitoreo y Control) resaltadas.}
  \label{fig:ciclo-pmi}
\end{figure}
 

%Problema: estimar, con métodos de IA/estadística, la probabilidad de desvío en tiempo, alcance, costo, %riesgos e incidentes respecto del último baseline aprobado de un proyecto. Fuentes (artefactos PMI), enfoque %(Bayes/Monte Carlo/series), métricas y valor para la gestión.
%Además de los enfoques estadísticos, se incorporarán modelos de ML para comparar su desempeño: boosting cuantílico (q10/50/90) para Δ_d y EAC, y modelos secuenciales (TCN/LSTM) sobre {SPI, CPI, EV, AC}. La comparación se hará con backtesting (en escalas de días) y métricas de error y calibración (MAE/RMSE, pinball, Brier/ECE). El objetivo es entregar pronósticos probabilísticos calibrados y alertas útiles para la gestión.



% ESTA ES LA GUÍA DE LA SECCIÓN "DESCRIPCIÓN" (OM)
